...
open class Directions: NSObject {
    ...
    fileprivate static func descriptiveError(_ json: JSONDictionary, response: URLResponse?, underlyingError error: NSError?) -> NSError {
        ...
        if let response = response as? HTTPURLResponse {
            ...
            case (429, _):
                if let timeInterval = response.allHeaderFields["x-rate-limit-interval"] as? TimeInterval, let maximumCountOfRequests = response.allHeaderFields["x-rate-limit-limit"] as? NSNumber {
                    let intervalFormatter = DateComponentsFormatter()
                    intervalFormatter.unitsStyle = .full
                    let formattedInterval = intervalFormatter.string(from: timeInterval)
                    
                    
                    let formattedCount = NumberFormatter.localizedString(from: maximumCountOfRequests,number: NumberFormatter.Style(rawValue: UInt(NSDecimalNoScale))!)
