...
@objc public protocol PXGoogleDirectionsDelegate {
	...
	*/
	@objc optional func googleDirectionsWillSendRequestToAPI(googleDirections: PXGoogleDirections, withURL requestURL: NSURL) -> Bool
	/**
	...
	*/
	@objc optional func googleDirectionsDidSendRequestToAPI(googleDirections: PXGoogleDirections, withURL requestURL: NSURL)
	/**
	...
	*/
	@objc optional func googleDirections(googleDirections: PXGoogleDirections, didReceiveRawDataFromAPI data: NSData)
	/**
	...
	*/
	@objc optional func googleDirectionsRequestDidFail(googleDirections: PXGoogleDirections, withError error: NSError)
	/**
	...
	*/
	@objc optional func googleDirections(googleDirections: PXGoogleDirections, didReceiveResponseFromAPI apiResponse: [PXGoogleDirectionsRoute])
}
...
public class PXGoogleDirections: NSObject {
	...
	public static var canOpenInGoogleMaps: Bool {
		return UIApplication.shared.canOpenURL(NSURL(string: "comgooglemaps://")! as URL) && UIApplication.shared.canOpenURL(NSURL(string: "comgooglemaps-x-callback://")! as URL)
	}
	...
	*/
	public var directionsAPIRequestURL: NSURL? {
		...
		if let f = from, let t = to {
			...
			if let requestURL = preparedRequest.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)  {
	...
	*/
	public func calculateDirections(completion: @escaping PXGoogleDirectionsRequestCompletionBlock)
	{
		...
		if let requestURL = directionsAPIRequestURL {
			...
			let runQuery = (delegate == nil ? true : (delegate!.googleDirectionsWillSendRequestToAPI?(googleDirections: self, withURL: requestURL) ?? true))
			...
			if runQuery {
				URLSession.shared.dataTask(with: requestURL as URL, completionHandler: { (data, response, error) -> Void in
					...
					// Notify delegate
					self.delegate?.googleDirections!(googleDirections: self, didReceiveRawDataFromAPI: data! as NSData)
					...
					if error == nil {
						...
						do {
							if let json = try JSONSerialization.jsonObject(with: data!, options: []) as? [String: AnyObject] {
								...
								if let st = json["status"] as? String {
									let status = PXGoogleDirectionsError.errorFromStatus(status: st)
									...
									if status.failed {
										...
										let err = self.prepareError(code: status)
										self.delegate?.googleDirectionsRequestDidFail?(googleDirections: self, withError: err)
												...
												if let legs = r["legs"] as? [[String: AnyObject]] {
													...
													for l in legs {
														...
														if let _ = l["steps"] as? [[String: AnyObject]] {
															currentLeg.steps = self.handleSteps(jsonData: l)
														}
									...
									let err = self.prepareError(code:
                                        .MissingStatusCode)
									self.delegate?.googleDirectionsRequestDidFail?(googleDirections: self, withError: err)
									completion(.Error(self, err))
									return
								...
								let err = self.prepareError(code: .BadJSONFormatting)
								self.delegate?.googleDirectionsRequestDidFail?(googleDirections: self, withError: err)
								completion(.Error(self, err))
								return
							}
						...
						DispatchQueue.main.async {
							...
							self.delegate?.googleDirections!(googleDirections: self, didReceiveResponseFromAPI: response)
							completion(.Success(self, response))
							return
						}
					} else {
						// Generic NSURLSession error: notify delegate and call completion block (with error value)
						self.delegate?.googleDirectionsRequestDidFail?(googleDirections: self, withError: error! as NSError)
						completion(PXGoogleDirectionsResponse.Error(self, error! as NSError))
						return
					}
				}).resume()
				// Notify delegate
				delegate?.googleDirectionsDidSendRequestToAPI?(googleDirections: self, withURL: requestURL)
			}
		} else {
			// Unable to create an API URL with the supplied arguments: notify delegate and call completion block (with error value)
			let err = prepareError(code: .BadAPIURL)
			delegate?.googleDirectionsRequestDidFail?(googleDirections: self, withError: err)
			completion(.Error(self, err))
			return
		}
	}
	...
	public func openInGoogleMaps(center center: CLLocationCoordinate2D?, mapMode: PXGoogleMapsMode?, view: Set<PXGoogleMapsView>?, zoom: UInt?, callbackURL: NSURL?, callbackName: String?, fallbackToAppleMaps: Bool = true) -> Bool {
		...
		if let f = from, let t = to {
			...
			if f.isSpecified() && t.isSpecified() {
				...
				params.append("saddr=\(f.description.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed) )")
				params.append("daddr=\(t.description.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed) )")
				...
				if let url = PXGoogleDirections.buildGoogleMapsURL(params: params, callbackURL: callbackURL, callbackName: callbackName) {
					...
					if fallbackToAppleMaps {
						...
						UIApplication.shared.openURL(NSURL(string: "https://maps.apple.com/\(p)")! as URL)
						return true
					}
				}
			}
		...
		if let steps = jsonData["steps"] as? [[String: AnyObject]] {
			for s in steps {
				...
				if let startLocation = s["start_location"] as? [String: CLLocationDegrees] {
					if let slLat = startLocation["lat"], let
                        slLng = startLocation["lng"] {
				...
				currentStep.travelMode = PXGoogleDirectionsMode.modeFromLabel(label: (s["travel_mode"] as? String) ?? "")
				// Distance
				...
				if let transitDetails = s["transit_details"] as? [String: AnyObject] {
					...
					if let line = transitDetails["line"] as? [String: AnyObject] {
						...
						if let iconUrl = NSURL(string: (line["icon"] as? String) ?? "") {
							if let data = NSData(contentsOf: iconUrl as URL) {
								currentStep.transitDetails?.line?.icon = UIImage(data: data as Data)
							}
						}
						// Agencies
						...
						if let vehicle = line["vehicle"] as? [String: String] {
							if let vName = vehicle["name"], let vType = vehicle["type"], let vIconURL = NSURL(string: vehicle["icon"] ?? "") {
								currentStep.transitDetails?.line?.vehicle = PXGoogleDirectionsTransitLineVehicle(name: vName, type: PXGoogleDirectionsVehicleType.vehicleTypeFromValue(value: vType), icon: nil)
								if let data = NSData(contentsOf: vIconURL as URL) {
									currentStep.transitDetails?.line?.vehicle?.icon = UIImage(data: data as Data)
								}
							}
						}
					}
					...
					if let _ = s["steps"] as? [[String: AnyObject]] {
						currentStep.steps = handleSteps(jsonData: s)
					}
				}
	...
	internal class func buildGoogleMapsURL(params params: [String], callbackURL: NSURL?, callbackName: String?) -> NSURL? {
		...
		if PXGoogleDirections.canOpenInGoogleMaps {
			...
			if let cbURL = callbackURL, let cbn = callbackName {
				...
				p.append("x-success=\(cbURL.absoluteString?.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed) )")
				p.append("x-source=\(cbn.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed) )")
			}
			return NSURL(string: "\(scheme)://\((p.count > 0) ? "?" + p.joined(separator: "&") : "")")!
		} else {
			return nil
		}
	}
