...
public class PXGoogleDirectionsRoute: NSObject {
	...
	*/
	public func drawOnMap(map: GMSMapView, approximate: Bool = false, strokeColor: UIColor = UIColor.red, strokeWidth: Float = 2.0) -> [GMSPolyline] {
	...
	*/
	public func drawOriginMarkerOnMap(map: GMSMapView, title: String = "", color: UIColor = UIColor.red, opacity: Float = 1.0, flat: Bool = false) -> GMSMarker? {
		var marker: GMSMarker?
		if let p = path {
			if p.count() > 1 {
				marker = drawMarkerWithCoordinates(coordinates: p.coordinate(at: 0), onMap: map, title: title, color: color, opacity: opacity, flat: flat)
			}
		}
		return marker
	}
	...
	- returns: The resulting `GMSMarker` object that was drawn to the map
	*/
	public func drawDestinationMarkerOnMap(map: GMSMapView, title: String = "", color: UIColor = UIColor.red, opacity: Float = 1.0, flat: Bool = false) -> GMSMarker? {
		var marker: GMSMarker?
		if let p = path {
			if p.count() > 1 {
				marker = drawMarkerWithCoordinates(coordinates: p.coordinate(at: p.count() - 1), onMap: map, title: title, color: color, opacity: opacity, flat: flat)
			}
		}
		return marker
	}
	...
	private func drawMarkerWithCoordinates(coordinates: CLLocationCoordinate2D, onMap map: GMSMapView, title: String = "", color: UIColor = UIColor.red, opacity: Float = 1.0, flat: Bool = false) -> GMSMarker {
