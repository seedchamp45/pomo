...
//  let alertSound = NSURL(fileURLWithPath: "alarm.m4a")
    //let path = Bundle.main.url(forResource: "alarm", withExtension:"m4a")!
    //let alertSound = URL(fileURLWithPath: )
    
    //NSURL(fileURLWithPath: Bundle.main.path(forResource: "alarm", ofType: "m4a")!)
  ...
  var currentVibrationWait: Float = 0.0
    ...
    override init() {
        let path = Bundle.main.path(forResource: "alarm", ofType: "m4a")
  ...
  private func activateVibrationRampup() {
    ...
    vibrationTimer = Timer.scheduledTimer(
      ...
      target: self,
      selector: #selector(AlarmSoundHelper.triggerVibration),
      userInfo: nil,
      repeats: false
    )
  }
